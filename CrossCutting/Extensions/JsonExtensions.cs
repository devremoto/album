...
", "\n").Trim()) : string.Format(tpl, prop.Name, string.Empty));
        ...
        public static string JsonRemoveProperty(this string json, string property)
        {
            var o = (JObject)JsonConvert.DeserializeObject(json);
            o.Property(property).Remove();
            return o.ToString();
        }
        public static string JsonSerialize<T>(this T obj, bool idented = false, bool convertEnum = false)
        {
            if (obj != null)
            {
                ...
                {
                    Formatting = idented ? Newtonsoft.Json.Formatting.Indented : Newtonsoft.Json.Formatting.None,
