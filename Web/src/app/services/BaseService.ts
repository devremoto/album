...
import { PagingModel } from '../components/pagination/paging-model';
import { UploadModel } from '../models/common/UploadModel';
...
export class BaseService<T> {
  ...
  getAll(): Observable<T[]> {
    return this.httpService.get(this.controller).pipe(
      map<any, T[]>(response => {
        this.emit(this.controller, response);
        return response;
      })
    );
  }
  ...
  getPage(page: PagingModel<T>): Observable<PagingModel<T>> {
    this.page = { ...page };
    delete (this.page.list);

    return this.httpService
      .get(this.controller, this.page)
      .pipe(
  ...
  put<TResult>(method: string, params?: any): Observable<TResult> {
    return this.httpService.put(`${this.controller}/${method}`, params).pipe(
      map<any, TResult>(response => {
        return response;
      })
    );
  ...
  delete<TResult>(method: string, params?: any): Observable<TResult> {
    return this.httpService.delete(`${this.controller}/${method}`).pipe(
      map<any, TResult>(response => {
        return response;
      })
    );
  ...
  apiUpload(files: any, entity?: any) {
  ...
  removeById(id: any) {
    return this.httpService.delete(`${this.controller}/${id}`).pipe(
      map(() => {
        this.emit(this.controller + '-remove', id);
      })
    );
  }
